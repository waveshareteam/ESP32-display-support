...
void user_bsp_Init(lv_ui *ui)
{
  ...
  xTaskCreate(color_user, "color_user", 2000, ui, 2, NULL); 
  xTaskCreate(GPIO_Test, "GPIO_Test", 3000, NULL, 2, NULL); //GPIO Test
}
void user_example(void *udata)
{
  ...
  if(sd_value != 0)
  {
    sprintf(sd_buff,"%.2f G",sd_value);
    lv_label_set_text(obj->screen_label_13, sd_buff);
  }
#endif
  for(;;)
  {
    ...
    {
      adc_test = stimes;
      adc_get_value(&adc_value,&adc_nc);
      if(adc_value)
      {
...
void GPIO_Test(void *pro)
{
  ...
  gpio_conf.pin_bit_mask = (0x01<<4) | (0x01<<13);
  gpio_conf.pull_down_en = GPIO_PULLDOWN_DISABLE;
  gpio_conf.pull_up_en = GPIO_PULLUP_ENABLE;
  ESP_ERROR_CHECK_WITHOUT_ABORT(gpio_config(&gpio_conf)); 
  for(;;)
